...
namespace estate_executor {
    int assemble_account_number(int secret_modifier) {
        return ((zhang::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier) + garcia::bank_number_part(secret_modifier)) + secret_modifier) % 10000;
    }

    int assemble_code(int secret_modifier) {
        int khan_total{khan::blue::code_fragment() + khan::red::code_fragment()};
        int zhang_total{zhang::blue::code_fragment() + zhang::red::code_fragment()};
        int garcia_total{garcia::blue::code_fragment() + garcia::red::code_fragment()};
        int total_fragments{khan_total + zhang_total + garcia_total};

        return total_fragments * secret_modifier;
    }
}